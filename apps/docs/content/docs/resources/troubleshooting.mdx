...
To catch this in CI, run `npx createcms generate --check` — it fails when the committed schema is stale. See the [CLI reference](/docs/reference/cli).
...
**Fix.** Return it, typed as `MultiTenantMiddlewareResult` — see [Multi-tenant → Installation](/docs/plugins/multi-tenant#add-it-to-your-config).
...
**Fix.** Install the optional `@jsquash/webp` fallback — see [Media optimize → Installation](/docs/plugins/media-optimize#add-the-client-plugin).
