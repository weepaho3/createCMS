...
`CMSQueryOptions` is `{ method?: string; query?: Record<string, unknown> }`, where `method` defaults to `'GET'`. Pass `options` as a thunk (`() => ({ query: {...} })`) to recompute it on every fetch — the only way to keep `query` or `method` reactive across refetches, since a plain object is captured once.

`CMSQueryState<T>` is `{ data, error, isPending, isRefetching, refetch }`.
