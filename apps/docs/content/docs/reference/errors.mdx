...
| `PROTECTED_BRANCH` | 403 | A direct content mutation was attempted on the protected default branch (`branchProtection.protectMain`) |
...
<Callout type="info">
  Plugins add their own codes (for example `TENANT_SLUG_REQUIRED`, `OPTIMIZATION_FAILED`). See each [plugin page](/docs/plugins).
</Callout>
