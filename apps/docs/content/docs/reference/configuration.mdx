...
| `authMiddleware` | `CMSMiddleware` | yes | | Resolves the user and permissions per request. `createCMS` throws without it; pass [`allowAnonymous()`](/docs/resources/security#authentication-is-required) to run with no auth on purpose. |
...
| `allowedMimeTypes` | `string[]` | `['image/png', 'image/jpeg', 'image/webp', 'image/gif', 'video/mp4', 'video/webm', 'application/pdf']` | Allowed MIME types (exact match, no wildcards). `image/svg+xml` is **excluded** by default — SVG can carry inline script (stored-XSS risk). Add it (or any other type) explicitly only if you serve assets cross-origin. See [Security → Media privacy](/docs/resources/security#media-privacy). |
...
`authMiddleware` is **required** — `createCMS` throws at construction without it, so auth is never silently absent. To run public/dev with no auth on purpose, pass `allowAnonymous()` (exported from `@createcms/core`), which authorizes every request; see [Security → Authentication is required](/docs/resources/security#authentication-is-required).
...
Enables live server-to-client delivery — notification push and A/B live results — over Server-Sent Events. **Upstash-only**: pass your Upstash Redis credentials. `@upstash/realtime` and `@upstash/redis` are optional peer dependencies. See [Realtime](/docs/concepts/realtime).
...
Requires `authMiddleware` to resolve the user from the request **cookie** — the realtime route authenticates each connection by session, so it must be same-origin with your app. Omit `realtime` to disable; notifications stay available through the `notifications.list` poll.
...
The default integration strategy for [`executeMerge`](/docs/reference/server-api), applied when a fast-forward is possible — that is, when the target branch has not diverged from the merge base.

| Value | Behavior |
| --- | --- |
| `'fast-forward'` | Move the target head straight to the source head, with no merge commit. The default. |
| `'merge-commit'` | Always record an explicit merge commit — git's `--no-ff` — so every integration stays visible in history. |

A **diverged** target always produces a merge commit regardless of this setting, and a merge with nothing to integrate is always a no-op fast-forward. Override the configured strategy per call with `executeMerge({ noFastForward })`: `true` forces a merge commit, `false` forces a fast-forward. See [Merges](/docs/concepts/merges#fast-forward-vs-merge-commit).
...
## Plugin schema columns

A plugin's [`schema`](/docs/plugins/building#add-a-database-table) declares tables with `definePluginSchema`. Each table's `columns` is a map of column keys to a `ColumnDefinition` (see `packages/cms/src/core/db/types.ts`); only `type` is required.

| Field | Type | Description |
| --- | --- | --- |
| `type` | scalar or `{ enum }` | **Required.** A scalar — `'text' \| 'boolean' \| 'integer' \| 'timestamp' \| 'jsonb' \| 'tsvector'` — or `{ enum: 'enumName' }` referencing an enum declared in the schema's `enums`. |
| `columnName` | `string` | SQL column name. Defaults to the snake_case of the column key. |
...
| `default` | `DefaultValue` | A default value (see below). |
| `defaultId` | `boolean` | Generate a prefixed id at insert time. Requires `defaultIdPrefix`. |
| `defaultIdPrefix` | `string` | Prefix for the id generated by `defaultId`. |
| `defaultNow` | `boolean` | Default to `now()` (for `timestamp` columns). |
| `jsonType` | `string` | TypeScript type applied to a `jsonb` column via Drizzle's `.$type<…>()`. |
| `references` | `object` | A foreign key (see below). |

`default` is a discriminated union on `kind`:

| Kind | Shape | Meaning |
| --- | --- | --- |
| `'literal'` | `{ kind: 'literal', value }` | A literal `boolean`, `number`, `string`, `string[]`, or `Record<string, unknown>`. |
| `'sql'` | `{ kind: 'sql', value: string }` | A raw SQL expression. |

`references` is `{ table, column, onDelete?, onUpdate? }`, where each action is one of `'cascade' \| 'restrict' \| 'no action' \| 'set null' \| 'set default'`.

**Indexes are table-level, not per-column.** Declare them in `TableDefinition.indexes` — a map of index keys to `{ columns, unique?, using?: 'btree' \| 'gin', where? }`.
...
const schema = definePluginSchema()({
  enums: {
    abTestStatus: { values: ['draft', 'running', 'paused', 'completed'] },
  },
  tables: {
    abTests: {
      tableName: 'ab_tests',
      indexPrefix: 'abt',
      columns: {
        id: { type: 'text', primaryKey: true, defaultId: true, defaultIdPrefix: 'abTest' },
        rootId: {
          type: 'text',
          notNull: true,
      ...
      // Indexes are table-level, keyed by name.
      indexes: {
