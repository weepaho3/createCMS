...
| `media` | `MediaConfig` | no | | S3-compatible storage config (see below). Optional — a content-only app can omit it; the media endpoints then throw `MEDIA_NOT_CONFIGURED` on use. |
...
`media` is optional. Omit it for a content-only app that never uploads — the media endpoints stay on the API but throw `MEDIA_NOT_CONFIGURED` when called. Add it as soon as you need uploads or asset serving.
