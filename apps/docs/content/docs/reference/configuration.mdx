...
| `notifications` | `false` | no | (enabled) | Set to `false` to fully disable notifications — no tables, no routes, and `client.notifications` / `cms.notify` absent from the types (see below). Only the literal `false` is accepted. |
...
Set to `false` to fully disable the notifications feature. When disabled, the notification tables are not generated (regenerate your schema after toggling), the routes never register, and `client.notifications` plus `cms.notify` are **absent from the inferred types**. Omit it to keep notifications enabled (the default). The option accepts only the literal `false`, so wiring it to a widened `boolean` (e.g. an env var) is a compile error rather than a silent trap where the types stay enabled but the runtime 404s. Independent of `realtime`.
