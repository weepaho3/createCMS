...
Governance for the default branch and the merge/publish approval gates. Every field is opt-in; an empty config preserves current behavior.

| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `protectMain` | `boolean` | `false` | Reject direct content mutations on the default branch (create/update/delete/move/duplicate of blocks, and `updateRoot`). Edits must happen on another branch and reach the default branch via a merge. `createRoot` is exempt. Throws `PROTECTED_BRANCH`. |
