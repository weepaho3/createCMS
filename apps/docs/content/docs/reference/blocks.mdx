...
<APIMethod
  ...
  returns={{
    ...
    reconstructed: {
      type: 'boolean',
      description: 'True when the commit was rebuilt from a partial snapshot.',
    },
    references: {
      type: 'Record<string, BlockTreeNode>',
      optional: true,
      description:
  ...
  params={{
    ...
    commitId: {
      type: 'string',
      description: 'A specific commit to read; defaults to the branch head.',
    },
    raw: {
      type: 'boolean',
      description:
...
<Callout type="info">
  A default read resolves the tree for RENDERING: variables are substituted and link values are rewritten to their resolved shape (`href` instead of the stored fields). That tree is not writable. For read-modify-write workflows (scripts, migrations, editors) request `raw: true` and edit the stored values; `updateBlocks` validates the stored shape only.
</Callout>

### Resolve an Unsaved Tree [#resolvetree]
...
<APIMethod
  ...
  params={{
    rootId: {
      type: 'string',
      required: true,
      description: 'The entry the tree belongs to (scope check only).',
    },
    branchId: {
      type: 'string',
      required: true,
      description: 'A branch of that entry (must exist; nothing is read from it).',
    },
    tree: {
      type: 'block tree node',
      required: true,
      description:
...
`resolveTree` never persists anything and creates no commit. Its response is the RENDERED view (variables substituted, links resolved) and is not writable; keep editing the raw working tree and save that with `updateBlocks` when you are done. Roots outside your scope (or of another collection) are rejected with `ROOT_NOT_FOUND`, exactly like `getBlockTree`.
...
<APIMethod
  ...
  returns={{
    ...
    blockId: {
      type: 'string',
      description: 'The id of the newly created block.',
    },
  }}
  params={{
    rootId: { type: 'string', required: true, description: 'The entry.' },
    ...
    type: {
      ...
      required: true,
      description: "One of the collection's block types.",
    },
    properties: {
      type: 'typed block properties',
      required: true,
      description: "The block's properties, typed from its block definition.",
    },
    position: {
      type: 'number',
      description: "Index in the parent's children (defaults to append).",
    },
    message: {
      type: 'string',
      description: 'Commit message (defaults to `Add {type} block`).',
    },
...
Save a whole tree in one commit: createCMS diffs the tree you send against the branch head and creates, updates, and deletes blocks to match. This is the batch save path your editor calls after local edits, instead of one call per change. Property values must be in their STORED shape; a tree read without `raw: true` carries resolved link values and is rejected with a `TYPE_MISMATCH` that names the affected properties.
...
<APIMethod
  ...
  params={{
    ...
    branchId: {
      type: 'string',
      required: true,
      description: 'The branch to write.',
    },
    tree: {
      type: 'block tree node',
      required: true,
...
<APIMethod
  ...
  params={{
    ...
    newParentBlockId: {
      type: 'string',
      required: true,
      description: 'Block to move it under.',
    },
    newIndex: {
      type: 'number',
      required: true,
    ...
    message: {
      type: 'string',
      description: 'Commit message (defaults to `Move block {blockId}`).',
    },
    expectedHeadCommitId: {
      type: 'string',
      description:
...
Clone a block subtree under an existing parent. `targetParentBlockId` is required — this endpoint only ever duplicates into a parent. To spin a subtree off into a brand-new top-level entry instead, use [`duplicateRoot`](/docs/reference/roots#duplicateroot).
...
<APIMethod
  ...
  returns={{
    ...
    blockId: { type: 'string', description: "The new copy's id." },
  }}
  params={{
    ...
    blockId: {
      type: 'string',
      required: true,
      description: 'Block (and its whole subtree) to duplicate.',
    },
    targetParentBlockId: {
      type: 'string',
      required: true,
      description: 'Parent for the copy.',
    },
    targetIndex: {
      type: 'number',
      description: "Index in the target parent's children (integer >= 0).",
    },
...
<APIMethod
  ...
  params={{
    ...
    blockId: {
      type: 'string',
      required: true,
      description: 'Block to delete (with all its descendants).',
    },
    message: {
      type: 'string',
      description: 'Commit message (defaults to `Delete block {blockId}`).',
    },
    expectedHeadCommitId: {
      type: 'string',
      description:
...
<APIMethod
  ...
  returns={{
    pageCount: {
      type: 'number',
      description: 'How many distinct live pages embed this reusable block.',
    },
    pages: {
      type: '{ rootId, collection, slug, occurrences }[]',
      description:
  ...
  params={{
    rootId: {
      type: 'string',
      required: true,
      description: 'Root id of the reusable block to inspect.',
    },
  }}
/>
