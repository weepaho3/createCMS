...
`GET /media/asset/{slug}` is a public redirect to the asset's object URL — the way to serve assets in content and on public pages. It honors `?format=webp|jpeg|png`, `?w=<width>` (resolving a pre-uploaded variant, falling back to the original), and `?download`. Private assets return `ASSET_ACCESS_DENIED`.
