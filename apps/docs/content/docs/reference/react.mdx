...
```ts
function createNotificationRouter<TActorUser>(routes: {
  [type]?: (n: TypedNotification<type>) => NotificationRoute;
...
The built-in core types (`mention`, `comment`, `mergeRequest*`, `approval*`, `published`, …) come with typed `meta` out of the box. To type your own `custom` notifications, augment `NotificationMetaMap`:
