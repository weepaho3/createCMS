...
Without `realtime`, nothing breaks: the `/realtime` route is never mounted, and notifications stay available through the durable [`notifications.list`](/docs/reference/notifications) poll — you just don't get live push, and there is no built-in polling hook.
...
The delivery model has two cost dimensions worth knowing before you scale it:

- **Connections.** Each subscriber holds one serverless invocation open for the life of its SSE stream. The platform or `@upstash/realtime` caps that stream's duration, and the client reconnects when it ends, so concurrent connections, not total users, drive invocation count.
- **Fan-out.** Each notification is one Upstash REST publish **per recipient**. Delivering an event to N users is N publishes, so a broad notification scales linearly with its audience.

Neither is a bottleneck for typical inboxes, but both are worth sizing for high concurrency or large broadcast audiences.
...
When `realtime` is set, every notification is pushed to its recipient's private channel automatically — all sources funnel through one dispatch. The hook seeds list + unread count from the `notifications.list` poll, then prepends live pushes de-duped by id; the provider replays anything missed across a reconnect. Pass the current user's `id` — the server authorizes it against the session, so a wrong id only yields the poll-seeded list, never another user's data.
...
Without `realtime` there is no live connection — read the durable list yourself with `client.notifications.list` and poll on your own cadence. The A/B [`useLiveResults`](/docs/plugins/ab-test) hook rides the **same** `RealtimeProvider` connection.
