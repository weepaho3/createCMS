...
<Callout type="info">
  The option accepts only the literal `false`, so wiring it to a widened `boolean` (e.g. an env var) is a compile error — the types and the runtime can never disagree. `notifications` and `realtime` are independent: A/B live results can use `realtime` with `notifications: false`.
</Callout>
