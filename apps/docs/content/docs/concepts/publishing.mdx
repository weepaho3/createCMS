...
- **Clearing a slug is not materialized.** If you clear a draft slug back to empty (for example, promoting an `allowRoot` page to the home page), publishing does not reset `roots.slug`; it keeps its last published value. Telling an explicitly cleared slug apart from one that was never set is a future enhancement.
