...
`getDiff` classifies the per-block changes of source against base (a two-way diff): each changed block gets `changeTypes` of `added`, `deleted`, `modified` (with per-property detail), `moved`, or `childrenReordered`. The move classification is identity-based — a block counts as `moved` only when it was reparented or truly reordered among its surviving siblings, not when its index shifted because another block was inserted or removed around it, and `childrenReordered` marks a parent only when the relative order of its surviving children changed. Alongside the flat list, `getDiff` returns an annotated copy of the draft tree that renders through your normal block components (see [Review changes visually](/docs/guides/visual-diff)). Its two sides are not limited to branches: each is a ref — a branch, a commit, or (target only) the entry's current publication — and when one ref is an ancestor of the other, the base **is** that ref, so the diff is an exact two-way comparison (a publish preview, or a commit against its parent). `checkConflicts` is the three-way detector: it returns the conflicting blocks, each with its source, target, and base version ids.
...
<Callout type="info">
  To run the full flow, see [Draft, review, and publish](/docs/guides/content-workflow). For signatures, see the [Merges API](/docs/reference/merges).
</Callout>
