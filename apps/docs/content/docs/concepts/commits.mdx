...
A row in `commits` holds the `rootId`, a `parentCommitId` (the previous commit), an optional `mergeSourceCommitId` (set only on merge commits), a `message`, and the author (`createdBy`). It does not store the tree inline. The tree lives in two tables:
