...
Every commit copies the parent's snapshot rows forward, so a write touches one snapshot row per block regardless of how few blocks changed. That write amplification is bounded by the entry's size (its block count), and its accumulated storage is bounded by [retention](/docs/concepts/data-retention): `keepDays` and `keepMinCommits` cap how many old snapshots survive. The reader already tolerates missing snapshots (it replays from the nearest survivor), so this is a deliberate lean toward fast reads.
...
[`getRootHistory`](/docs/reference/roots#getroothistory) returns an entry's commits across every branch, newest first, each already labelled with its `type` and the [branch](#branch-attribution) it was created on. Every row also carries `parents` (one commit id, or two for a merge), `isPublished`, `createdAt`, and `createdBy`.
