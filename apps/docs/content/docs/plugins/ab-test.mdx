...
| `useLiveResults` | `({ testId, initial }) => { results, isLive }` | React hook: live dashboard results. |

<Callout type="info">
  `useLiveResults` streams result deltas over the [realtime](/docs/concepts/realtime) transport on the public `ab:live:<testId>` channel, so the dashboard updates live. Delivery is decoupled from the analytics storage adapter — it works with the Postgres or Upstash adapter. It requires a `realtime` transport configured on the server; without it, it falls back to the `getResults` poll.
</Callout>
