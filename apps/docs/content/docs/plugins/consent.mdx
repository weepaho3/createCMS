...
Neither `consent()` nor `consentClient()` takes configuration options; the client reads Consent Mode v2 commands automatically from the CMP (see Usage).
...
| --- | --- | --- | --- |
| `mapping` | `C15tCategoryMapping` | `DEFAULT_C15T_MAPPING` | Maps c15t consent categories to Consent Mode v2 signals (`measurement` → `analytics_storage`; `marketing` → `ad_storage`, `ad_user_data`, `ad_personalization`). |
