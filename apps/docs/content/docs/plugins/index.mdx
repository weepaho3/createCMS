...
Plugins extend the CMS without forking it. A server plugin can add endpoints, hooks, database schema, request-scoped query conditions, and error codes; a client plugin adds actions to `createCMSClient`. `@createcms/core` ships with five.
