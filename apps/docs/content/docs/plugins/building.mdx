...
After `createcms generate`, `abTestWinner` is a valid `notification_type`, and the app's `createNotificationRouter<typeof cms>({ abTestWinner: (n) => ({ href: `/experiments/${n.meta.testId}` }), … })` routes it with `meta` fully typed.
