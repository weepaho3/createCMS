...
`definePluginSchema` declares tables (and enums) that merge into your generated schema. Each table is a plain object — see the [column reference](/docs/reference/configuration#plugin-schema-columns) for the field types:

```ts title="plugins/page-views.ts"
import { definePlugin, definePluginSchema } from '@createcms/core';

// Curried: call `definePluginSchema()` (no type arg when you only declare your
// own tables), then pass the schema. Use `definePluginSchema<CoreTables>()` when
// you `extend` core tables so the extend keys are type-checked.
  ...
  tables: {
    pageViews: {
      ...
      columns: {
        ...
        // `default` is a discriminated union, not a raw scalar.
        count: {
          type: 'integer',
          notNull: true,
...
// `definePlugin` keeps `id` a literal (so `cms.api.pageViews` is a real
// namespace, not a string index signature) and preserves endpoint contributions.
export function pageViews() {
  return definePlugin({ id: 'pageViews', schema });
}
```
...
`$ERROR_CODES` declares the errors your plugin throws. On a **server** plugin they merge into `cms.$ERROR_CODES` (the server instance) — not the client's — so server-side consumers can branch on a stable `code`. Throw with better-call's `APIError`:

```ts title="plugins/page-views.ts"
import { APIError } from 'better-call';

const $ERROR_CODES = {
  PAGE_VIEWS_ROOT_NOT_FOUND: { status: 404, message: 'Root not found' },
} as const;

// inside a handler:
throw new APIError(404, {
  code: 'PAGE_VIEWS_ROOT_NOT_FOUND',
  message: $ERROR_CODES.PAGE_VIEWS_ROOT_NOT_FOUND.message,
});
```

<Callout type="info">
  Prefix plugin codes with the plugin name (`PAGE_VIEWS_ROOT_NOT_FOUND`, not `ROOT_NOT_FOUND`) so they don't collide with a [core code](/docs/reference/errors) or another plugin's.
</Callout>
...
async onRequest(request, ctx) {
  ...
  // The A/B plugin caps its one anonymous write path, POST /abTest/trackEvent.
  if (request.method === 'POST' && url.pathname.endsWith('/abTest/trackEvent')) {
...
- **`realtimeEvents`** — a Zod map of events your plugin publishes over the shared [realtime](/docs/concepts/realtime) transport (the transport itself is event-agnostic). Use the schemas to validate on publish and to parse on the client; the CMS does not currently thread them into a typed `cms.realtime.publish` or typed client subscriptions. (Requires `realtime` configured.)
...
export function pageViewsClient(): CMSClientPlugin {
  return {
    ...
    $ERROR_CODES: { PAGE_VIEWS_ROOT_NOT_FOUND: { status: 404, message: 'Root not found' } },
  };
}
```
...
- `realtimeEvents` → the realtime event registry your plugin publishes;
- server `$ERROR_CODES` → `cms.$ERROR_CODES`; client `$ERROR_CODES` → `client.$ERROR_CODES`.
...
| `atomListeners` | `CMSAtomListener[]` — invalidate query atoms after a mutation. Each `{ matcher, signal, callback? }` toggles a signal atom so subscribed `createCMSQuery` atoms refetch. |
