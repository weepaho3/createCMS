...
If `mr.hasConflicts` is true, drive a conflict UI from `getDiff` and settle each one through `applyConflictResolutions` (as `source`, `target`, or a `manual` version) before merging.
...
- **Branch, merge request, and approval views.** Lists work in flight with `listBranches` and `listMergeRequests`, shows a diff with `getDiff` and `checkConflicts`, and drives the review with `createMergeRequest`, `applyConflictResolutions`, `requestApproval`, `submitApproval`, `submitRejection`, and `executeMerge`. Thread discussion on top with `createCommentThread` and `listCommentThreads` (see [Review with comments](/docs/guides/review-comments)).
