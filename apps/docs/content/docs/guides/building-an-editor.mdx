...
If you batch edits client-side and save the whole tree at once instead of one mutation per change, use `updateBlocks` (`body: { rootId, branchId, tree }`). It diffs the tree against the branch head and creates, updates, and deletes blocks in a single commit; pass `expectedHeadCommitId` (the commit you loaded the tree at) so a concurrent write fails with `HEAD_MISMATCH` rather than clobbering it. See [`updateBlocks`](/docs/reference/server-api#updateblocks) for the full contract.
