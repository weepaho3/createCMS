...
createCMS is headless: it ships the versioned content API (branches, merge requests, approvals, publishing, media, notifications) but no editor UI of its own. The headless editor primitives live in `@createcms/react`; the surrounding UI you bring yourself. For copy-paste styled field chrome, see [Editor UI](/docs/guides/editor-ui).

This guide gives a recommended shape for that UI by mapping each editor surface to the API method that backs it, so you can build a dashboard, a block editor, and a review flow without inventing your own conventions.
