...
`getDiff` compares the source branch against the **common ancestor** it shares with the target (the merge base) — so changes that happened only on the target never show up as noise. See [Merges](/docs/concepts/merges) for the three-way model behind it.
