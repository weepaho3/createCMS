...
`fields={cmsFields}` and wrap with `CmsSourcesProvider`. `EditorShell`
provides palette, outline, and inspector; `FormSurface` is the padded form
for the selected block (page fields when nothing is selected). JSON debug
