...
`getPublishedContent` returns one entry per published branch in `variants` (see [Publishing](/docs/concepts/publishing)). With no A/B test there is exactly one, so `variants[0]` is the page. With the [A/B testing plugin](/docs/plugins/ab-test), resolve the visitor's branch and select it with `pickVariant` — see [A/B testing → Usage](/docs/plugins/ab-test#variant-selection).
