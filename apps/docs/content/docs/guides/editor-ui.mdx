...
`mode="form"` skips `Canvas.Root` and keeps the same document wiring. Palette, outline, inspector, and the device toggle still come from `EditorShell` — form mode is not a second shell. The [form-only demo](/demo/editor/form) is this path: `CmsEditor mode="form"` with a `Collapsible` JSON dump as children, not a bare `Editor.Root` + `Form` composition.
...
| `mode` | no | `'canvas'` | `'canvas'` or `'form'`. Form skips `Canvas.Root` but still uses `EditorShell` chrome (palette, device toggle, inspector). |
...
| `children` | no | — | Rendered inside `Editor.Root` after the shell (demo extras such as a JSON `Collapsible`). |
