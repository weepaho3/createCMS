...
`mode="form"` skips `Canvas.Root` and keeps the same document wiring. Palette, outline, inspector, and the device toggle still come from `EditorShell` — form mode is not a second shell.
...
**Styled container.** Wrap the block's fields with the styled `Form` shell. Inner fields remain the headless primitives from `@createcms/react` unless you pass `fields={cmsFields}` (shadcn `Input` / `Textarea` / `Select` and CMS pickers). `FormSurface` is a padded selected-block form for `mode="form"` and the form-only demo.
