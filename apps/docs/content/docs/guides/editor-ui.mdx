...
| `EditorPalette`   | `SidebarGroup` of `Canvas.PaletteItem` rows with grip affordance (insert)                     | `sidebar`, `editor-canvas`                         |
| `EditorOutline`   | Tree rows on `SidebarMenuButton` + `Editor.OutlineItem`, move `DragHandle`, chevron collapse, type icon | `sidebar`, `collapsible`, `editor-canvas`          |
...
| `FieldRing`     | Focused field outline when the field's block is not selected         |
