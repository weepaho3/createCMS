...
Call the typed API directly on the server. No client fetch is needed for published content. Use a catch-all route and look up by **`path`** — the full published path, with a leading slash — so nested pages resolve too:
...
export default async function Page({
  ...
  const path = `/${(slug ?? []).join('/')}`; // '/' for home, '/blog/post' nested
...
`getPublishedContent` looks content up by `path` (the full path), by `slug` (a single stored slug segment, no leading slash), or by `rootId`. Prefer `path` for URL-driven routing; it is the only one that resolves nested pages.
