...
export const approvalStatusEnum = cmsSchema.enum('approval_status', [
  'pending',
  'approved',
  'rejected',
]);

export const assetStatusEnum = cmsSchema.enum('asset_status', [
  'private',
  'public',
]);

export const commentMessageTypeEnum = cmsSchema.enum('comment_message_type', [
  'comment',
  'system',
]);

export const commentSystemTypeEnum = cmsSchema.enum('comment_system_type', [
  'threadResolved',
  'threadReopened',
]);

export const commentThreadStatusEnum = cmsSchema.enum('comment_thread_status', [
  'open',
  'resolved',
]);

export const commentThreadTargetEnum = cmsSchema.enum('comment_thread_target', [
  'mergeRequest',
  'block',
]);

export const conflictResolutionEnum = cmsSchema.enum('conflict_resolution', [
  'source',
  'target',
  'manual',
]);

export const contentUsageTargetEnum = cmsSchema.enum('content_usage_target', [
  'asset',
  'variable',
  'reference',
  'link',
...
export const mergeRequestStatusEnum = cmsSchema.enum('merge_request_status', [
  'open',
  'merged',
  'closed',
]);

export const notificationTypeEnum = cmsSchema.enum('notification_type', [
  'mention',
  'comment',
  'threadResolved',
  'threadReopened',
...
export const redirectEndpointTypeEnum = cmsSchema.enum(
  'redirect_endpoint_type',
  ['page', 'path'],
);

export const releaseStatusEnum = cmsSchema.enum('release_status', [
  'draft',
  'published',
]);

export const scheduledPublicationActionEnum = cmsSchema.enum(
  'scheduled_publication_action',
  ['publish', 'unpublish'],
);
...
export const approvals = cmsSchema.table(
  'approvals',
  {
    id: text('id')
      .primaryKey()
      .$defaultFn(() => newId('approval')),
    mergeRequestId: text('merge_request_id').references(
      () => mergeRequests.id,
      { onDelete: 'cascade' },
    ),
  ...
  (table) => [
    ...
    uniqueIndex('approvals_target_reviewer_unique').on(
      table.mergeRequestId,
      table.branchId,
      table.commitId,
      table.requestedReviewer,
    ),
  ],
);
...
export const assets = cmsSchema.table(
  'assets',
  {
    ...
    variantOf: text('variant_of').references((): AnyPgColumn => assets.id, {
      onDelete: 'set null',
    }),
...
export const commentMentions = cmsSchema.table(
  'comment_mentions',
  {
    id: text('id')
      .primaryKey()
      .$defaultFn(() => newId('commentMention')),
    messageId: text('message_id')
      .notNull()
      .references(() => commentMessages.id, { onDelete: 'cascade' }),
  ...
  (table) => [
    ...
    uniqueIndex('cmn_message_user_unique').on(
      table.messageId,
...
export const commentMessages = cmsSchema.table(
  'comment_messages',
  {
    id: text('id')
      .primaryKey()
      .$defaultFn(() => newId('commentMessage')),
    threadId: text('thread_id')
      .notNull()
      .references(() => commentThreads.id, { onDelete: 'cascade' }),
    parentMessageId: text('parent_message_id'),
    authorId: text('author_id'),
    messageType: commentMessageTypeEnum('message_type')
      .notNull()
      .default('comment'),
...
export const contentUsages = cmsSchema.table(
  ...
  (table) => [
    uniqueIndex('cu_version_target_prop_unique').on(
      table.blockVersionId,
      table.targetKind,
      table.targetKey,
      table.propertyKey,
    ),
...
export const mergeConflicts = cmsSchema.table(
  'merge_conflicts',
  {
    ...
    targetVersionId: text('target_version_id').references(
      () => blockVersions.id,
    ),
  ...
  (table) => [
    ...
    uniqueIndex('mc_merge_block_unique').on(
      table.mergeRequestId,
      table.blockId,
    ),
  ],
...
export const mergeRequests = cmsSchema.table(
  ...
  (table) => [
    ...
    uniqueIndex('mr_open_source_target_unique')
      .on(table.sourceBranchId, table.targetBranchId)
...
export const redirects = cmsSchema.table(
  ...
  (table) => [
    index('rdr_collection_source_path_idx').on(
      table.collection,
      table.sourcePath,
    ),
...
export const scheduledPublications = cmsSchema.table(
  'scheduled_publications',
  {
    id: text('id')
      .primaryKey()
      .$defaultFn(() => newId('scheduledPublication')),
    rootId: text('root_id')
      .notNull()
      .references(() => roots.id, { onDelete: 'cascade' }),
    branchId: text('branch_id')
...
export const templates = cmsSchema.table(
  ...
  (table) => [
    index('templates_collection_block_prop_idx').on(
      table.collection,
      table.blockType,
      table.propertyKey,
    ),
    index('templates_collection_idx').on(table.collection),
    index('templates_collection_block_idx').on(
      table.collection,
      table.blockType,
    ),
  ],
...
export const templateVariableUsages = cmsSchema.table(
  ...
  (table) => [
    uniqueIndex('tvu_key_template_unique').on(
      table.variableKey,
      table.templateId,
    ),
...
export const variables = cmsSchema.table(
  ...
  (table) => [index('variables_key_idx').on(table.key)],
);
...
export type NewTemplateVariableUsage =
  typeof templateVariableUsages.$inferInsert;
