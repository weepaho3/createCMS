...
describe('media.getAssetUrlAuthenticated (authenticated endpoint)', () => {
  it('returns a signed URL for a public asset', async () => {
    const { cms, db, s3 } = await setupTestCMS({ withS3: true });

    const [asset] = await db
      .insert(assets)
      .values({
        slug: 'public-auth.png',
        mimeType: 'image/png',
        size: 1024,
        objectKey: 'public-auth.png',
        status: 'public',
      })
      .returning();

    const result = await cms.api.media.getAssetUrlAuthenticated({
      body: { assetSlug: asset.slug },
    });

    expect(result.url).toContain('X-Amz-');
    expect(result.status).toBe('public');
    // Public assets get 1 hour TTL
    expect(result.expiresAt - Date.now()).toBeGreaterThan(3500 * 1000);

    await s3.cleanup();
  });

  it('returns a signed URL for a private asset (shorter TTL)', async () => {
    const { cms, db, s3 } = await setupTestCMS({ withS3: true });

    const [asset] = await db
      .insert(assets)
      .values({
        slug: 'private-auth.png',
        mimeType: 'image/png',
        size: 1024,
        objectKey: 'private-auth.png',
        status: 'private',
      })
      .returning();
    ...
    // Private assets get 5 minute TTL
    expect(result.expiresAt - Date.now()).toBeLessThan(400 * 1000);
  ...
  it('throws when asset does not exist', async () => {
    ...
    await expect(
      cms.api.media.getAssetUrlAuthenticated({
        body: { assetSlug: 'nonexistent.png' },
      }),
    ).rejects.toThrow(/Asset not found/i);
