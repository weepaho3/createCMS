 ...
 * the TS AST. `CMSDefinition` is a plain object type with no runtime value to
 * inspect, so the AST is the only place its keys exist.
 */
 ...
 * optional property cannot disagree.
 */
const TABLES: {
  label: string;
  type: string;
...
describe.each(TABLES)(
  'docs coverage: $label',
  ({ label, type, table, requiredColumn }) => {
    ...
    const members = (): TypeMember[] => typeMembers(sourceFile, type);
