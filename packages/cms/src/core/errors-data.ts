 ...
 *
 * This module intentionally has NO runtime dependencies (in particular it does
 * NOT import `better-call`), so it is safe to pull into a browser bundle. The
 * server-only `CMSError` class (which extends better-call's `APIError`) lives in
 * `./errors`, which re-exports everything here to keep its public API stable.
 */
