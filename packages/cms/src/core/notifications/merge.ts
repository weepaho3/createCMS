...
/**
 * Reconcile a seed/reconcile poll into the current state WITHOUT clobbering live
 * pushes. The poll is the durable source of truth, but its snapshot is taken
 * server-side and may predate a push that landed on the wire mid-flight — a
 * wholesale `setState({ notifications: res.notifications, ... })` would drop that
 * push until the next poll. Instead we union: keep any local item that is a
 * genuine mid-poll PUSH (strictly newer than everything the poll returned, so it
 * can only have arrived after the server took its snapshot) ahead of the durable
 * list, take the server's `unreadCount` as authoritative for the durable set, and
 * add back the unread of those pushes so the badge never under-counts.
 *
 * The freshness gate matters: a local item the poll omitted is NOT always a push.
 * Once the list is longer than the poll's `limit`, an item that scrolled past the
 * page window is also absent from the poll — but it is OLDER than the page and
 * already inside the server's total `unreadCount`, so re-adding it would
 * double-count the badge and yank a stale row to the top. Only items newer than
 * the newest polled row are real races. Empty poll → no reference point, so trust
 * the poll and keep nothing extra (a real push is persisted server-side and
 * reappears on the next reconcile). Pure and de-duped by id.
 */
