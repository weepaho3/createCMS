/**
 * Pure, dependency-free CMS error data.
 *
 * This file is a package export entry (`@createcms/core/errors-data`) so
 * bunchee emits it as its own chunk. It must stay free of React and of
 * `'use client'`: Next.js stubs `'use client'` shared chunks on the server,
 * and if `CMS_ERRORS` lives in one of those, `CMSError` throws TypeError on
 * `def.status`. The server-only `CMSError` class (which extends better-call's
