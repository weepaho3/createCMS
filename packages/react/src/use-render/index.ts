// Local copy of shadcn's `useRender` (packages/react/src/use-render), itself
// adapted from Base UI's `useRender` — MIT, © MUI / © shadcn (see
// THIRD_PARTY_NOTICES). Kept in-tree so the primitives carry no runtime
// dependency; drop it if Base UI ships a standalone package.
...
type RenderProp<
  TElement extends React.ElementType,
  TState extends RenderState,
> = React.ReactElement | RenderFunction<TElement, TState>;
...
> = React.ComponentPropsWithRef<TElement> & {
  render?: RenderProp<TElement, TState>;
};
...
export { composeRefs, getStateAttributes, mergeProps, useRender };
export type {
  RenderProp,
  RenderState,
  StateAttributesMapping,
