...
| `Canvas.FieldRing`     | `div`           | `render`, div props. Sized from the focused field rect (own-block only). Hidden when that field belongs to the selected block so SelectionRing is the single calm ring.                                                                                                  | `data-editor-field-ring`, `data-block-type`, `data-can-move`, `data-can-delete`, `data-unresolved`        |
| `Canvas.BlockToolbar`  | `div`           | `align`, `side`, `offset`, `render`, div props. Consumer supplies buttons. Shown in `edit` and `select`. When `side="top"` would paint above the overlay clip (`overflow: hidden`), the toolbar pins inside the visible block instead.                                   | `data-editor-block-toolbar`, `data-side`, `data-block-type`                                               |
...
| `Canvas.DragHandle`    | `button`        | `blockId` (required), `render`, button props. Move an existing block by pointer drag. Shown only in `edit`. Overlay siblings inset into the visible block so overlay `overflow: hidden` does not clip the handle; inside `Canvas.BlockToolbar` the handle stays in flow. | `data-editor-drag-handle`, `data-dragging` on the handle while its block is the move session              |
...
| `rect.test.ts`                 | `unionRects`, visible intersection / overlay clip helpers, readonly skip, nested field key own-block lookup.                                                                                                                                                                                                                      |
| `overlay.test.tsx`             | Overlay portal into host, `useBlockRect` callable, SelectionRing vs FieldRing stacking, HoverRing suppression.                                                                                                                                                                                                                    |
...
| `dnd-parts.test.tsx`           | DragHandle / PaletteItem / DropIndicator / DragPreview throw outside Root; preview uses translate3d during drag; overlay handle insets with pointer-events; palette click insert; click no-op when placement fails; click after drag; drag threshold; inline-edit does not block the handle; escape cancel; pointerup off handle. |
...
| `insert.browser.test.tsx`      | Chromium: column / row / grid insert lines, empty container box, toolbar side / align, first-block toolbar stays hittable, insert disabled gating, select mode without insert.                                                                                                                                                    |
| `dnd.browser.test.tsx`         | Chromium: palette drop into empty stack, column and row sibling moves, overlay-anchored handle move, escape cancel, auto-scroll, touch drag, forbidden placement, focus after drop, select mode without drag.                                                                                                                     |
| `provider.browser.test.tsx`    | Chromium: sidebar palette drag into the canvas under a Provider; outline-style DragHandle moves without add (canvas or outline-row drop); two canvases under one provider resolve drops on the hovered editable surface. |
